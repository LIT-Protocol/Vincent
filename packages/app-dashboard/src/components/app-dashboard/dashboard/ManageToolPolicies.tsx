...
import { LazyInput } from '@/components/app-dashboard/lazyInput';
import { LazySelect } from '@/components/app-dashboard/lazySelect';
import { ToolPolicyWithId, PolicyWithId, ToolPolicyManagerProps, VersionInfo } from '@/types';
    ...
    try {
      ...
      const toolPolicyParameterTypes = validToolPolicies.map((tool) =>
        tool.policies.map((policy) =>
          policy.parameters
            .filter((param) => !!param.name && param.name.trim() !== '')
            .map((param) => mapTypeToEnum(param.type as string)),
        ),
      );
        ...
        allParameters.push(
          <div key={param._id} className="flex gap-2 items-start mb-2">
            <div className="flex-1 grid grid-cols-2 gap-2">
              ...
              />
              <LazySelect
                initialValue={param.type as string}
                onUpdate={(value) =>
