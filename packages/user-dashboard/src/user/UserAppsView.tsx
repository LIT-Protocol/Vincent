      ...
      <div className="flex justify-between items-center mt-4 pt-2 border-t border-gray-100">
        <div className="flex items-center">
          ...
          <span className={`px-2 py-0.5 text-xs rounded-full ${
            ...
            {app.deploymentStatus === 0 ? 'DEV' : 
              app.deploymentStatus === 1 ? 'TEST' : 'PROD'}
          </span>
        </div>
      </div>
    </div>
...
export default function UserAppsView({ userPKP, sessionSigs, agentPKP }: UserAppsViewProps) {
  ...
  useEffect(() => {
    ...
    async function fetchUserApps() {
      ...
      try {
        ...
        const appViewContract = new ethers.Contract(
          contractAddress,
          APP_VIEW_FACET_ABI,
          provider
        );
        ...
        const appDetailsPromises = appIds.map(async (appId: BigNumber) => {
          try {
            ...
            const showUpgradePrompt = currentVersion !== null && 
  ...
  const handleAppUpgrade = useCallback(async (appId: string) => {
    ...
    try {
      ...
      const currentVersion = appToUpgrade.version !== undefined ? 
        Number(appToUpgrade.version) : null;
      
      const latestVersion = appToUpgrade.latestVersion !== undefined ? 
        Number(appToUpgrade.latestVersion) : null;
