...
import { updateAppVersion } from '../components/user/userAppsUtils';
...
export async function handleAppUpgrade({
  ...
  try {
    ...
    const currentVersion = appToUpgrade.version !== undefined ? Number(appToUpgrade.version) : null;
    ...
    const updatedApps = updateAppVersion(apps, appId, Number(latestVersion));
