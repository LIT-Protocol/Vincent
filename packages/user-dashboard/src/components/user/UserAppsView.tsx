...
import { handleAppUpgrade } from '@/components/user/appUpgradeUtils';
...
export default function UserAppsView({ userPKP, sessionSigs, agentPKP }: UserAppsViewProps) {
  ...
  // Handle app upgrade
  const onAppUpgrade = useCallback(
    async (appId: string) => {
      if (!userPKP || !sessionSigs || !agentPKP) {
        showErrorWithStatus('Missing required authentication. Please reconnect your wallet.');
        return;
      ...
      const result = await handleAppUpgrade({
        appId,
        userPKP,
        agentPKP,
        sessionSigs,
      ...
      if (result.isSuccess && result.updatedApps) {
        setApps(result.updatedApps);
      }

      setIsUpgrading(false);
      setUpgradingAppId(null);
    },
    [userPKP, agentPKP, sessionSigs, apps, showStatus, showErrorWithStatus],
  );
