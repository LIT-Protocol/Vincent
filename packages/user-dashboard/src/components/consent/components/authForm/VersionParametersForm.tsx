...
export default function VersionParametersForm({
  ...
  const getToolFriendlyName = (toolIndex: number) => {
  ...
  const getPolicyFriendlyName = (policyIndex: number) => {
    ...
    <div className="mb-6">
      ...
      <div className="space-y-3">
        {toolsData.map((tool) => {
          ...
          const toolName = getToolFriendlyName(tool.toolIndex);
              ...
              >
                {tool.policies.length === 0 ? (
                  ...
                  tool.policies.map((policy) => {
                    ...
                    const policyName = getPolicyFriendlyName(policy.policyIndex);
