...
export const userOpV070Schema = userOpBaseSchema.extend({
  ...
  paymasterVerificationGasLimit: z
    ...
    .regex(/^0x([1-9a-fA-F]+[0-9a-fA-F]{0,15})|0$/)
    .optional()
    .describe('The gas limit for paymaster verification (required if paymaster exists).'),
  ...
  paymasterPostOpGasLimit: z
    ...
    .optional()
    .describe(
      'The amount of gas to allocate for the paymaster post-op code (required if paymaster exists)',
    ),
});
