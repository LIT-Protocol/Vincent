...
import { isRelayLinkExecuteAddress } from './relayLink';
...
interface DecodeRelayLinkOrERC20Result {
  kind: 'relay_link' | 'erc20_approval';
  fn?: string;
  args?: readonly unknown[] | undefined;
}

interface DecodeRelayLinkOrERC20Failure {
  kind: 'blocked';
  reason: string;
}

// --------------- Decode Relay.link + ERC20 and verify policy -------------------
function decodeRelayLinkOrERC20(
  call: LowLevelCall,
  relayLinkExecuteAddress: Address,
  chainId: number,
  additionalAllowedTargets: Address[] = [],
): DecodeRelayLinkOrERC20Result | DecodeRelayLinkOrERC20Failure {
  // Check if target is Relay.link execute contract or in additional allowed targets
  const isAllowedTarget =
    isRelayLinkExecuteAddress(call.to, chainId) ||
    additionalAllowedTargets.some((addr) => addr.toLowerCase() === call.to.toLowerCase());

  if (isAllowedTarget) {
    // For Relay.link, we accept any valid transaction to the execute contract or allowed targets
    // The actual validation of the call data happens in the simulation
    return { kind: 'relay_link' } as const;
  }

  // Try ERC20 approve-ish (spender must equal Relay.link execute contract)
  try {
    ...
    if (df.functionName === 'approve' || df.functionName === 'increaseAllowance') {
      ...
      if (!isAddressEqual(spender, relayLinkExecuteAddress)) {
        return {
          kind: 'blocked',
          reason: `ERC20 approval to non-Relay.link spender ${spender}`,
        } as const;
      }
...
const evaluateCallAgainstPolicy = ({
  ...
  const res = decodeRelayLinkOrERC20(
    call,
    relayLinkExecuteAddress,
    chainId,
    additionalAllowedTargets,
  ...
  if (res.kind === 'relay_link') {
    // Relay.link transactions are validated via simulation
    // No additional policy checks needed here
  }
