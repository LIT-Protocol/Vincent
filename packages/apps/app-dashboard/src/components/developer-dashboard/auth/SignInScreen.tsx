...
export function SignInScreen() {
  ...
  // Auto-trigger sign-in when wallet is connected but not authenticated
  useEffect(() => {
    if (isWalletConnected && !isAuthenticated && !isSigningIn && !error) {
      signIn();
    }
  }, [isWalletConnected, isAuthenticated, isSigningIn, error, signIn]);
  ...
  return (
    <div
      className={cn(
        'min-h-screen min-w-screen flex items-center justify-center transition-colors duration-500 relative z-10',
      )}
    >
      <div className="w-full max-w-md mx-4">
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/20 rounded-2xl p-8 shadow-xl">
          {/* Logo */}
