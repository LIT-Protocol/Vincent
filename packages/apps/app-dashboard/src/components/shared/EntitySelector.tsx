...
export function EntitySelector({
  ...
  // FIXME: Actually use setEntities
  setEntities(['temp']);

  // Use the actual API hooks
  // FIXME: Have the vincentApiClient export the lazy hooks
  ...
  useEffect(() => {
    const fetchEntities = async () => {
      try {
        ...
        // FIXME: let result;
        if (entityType === 'tool') {
          // FIXME: result = await listAllTools().unwrap();
        } else {
          // FIXME: result = await listAllPolicies().unwrap();
        }

        // FIXME: setEntities(result);
      } catch (err) {
    ...
    fetchEntities();
  }, [entityType]); // FIXME: listAllTools, listAllPolicies
