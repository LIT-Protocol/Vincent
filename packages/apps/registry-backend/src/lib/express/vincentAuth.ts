...
export type VincentAuthHandler = (
  fn: (
    req: Request & RequestWithVincentUser,
    res: Response,
    next: NextFunction,
  ) => void | Promise<void>,
...
/**
 * Creates SIWE authentication middleware and handler.
 * The middleware verifies the Authorization header containing a base64-encoded SIWE message and signature.
 * If valid, it adds the wallet address to req.vincentUser.
 */
function createSiweAuthMiddleware() {
  ...
  const middleware = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      if (!message || !signature) {
        ...
        res.status(401).json({
          message: 'Invalid authentication token',
          error: 'Payload must contain both message and signature',
        });
        return;
      }
  ...
  const handler: VincentAuthHandler = (fn) => {
    return (req, res, next) => {
      return fn(req as Request & RequestWithVincentUser, res, next);
    };
  };
...
// Create and export the middleware - called once at module load
export const { middleware: requireVincentAuth, handler: withVincentAuth } =
  createSiweAuthMiddleware();
