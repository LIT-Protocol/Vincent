import { createVincentPolicy, solana } from '@lit-protocol/vincent-ability-sdk';
...
export const vincentPolicy = createVincentPolicy({
  ...
  precheck: async ({ abilityParams, userParams }, { allow, deny }) => {
    ...
    const transaction = solana.deserializeTransaction(serializedTransaction);

    const verification = await solana.verifyBlockhashForCluster({
      transaction,
      cluster,
