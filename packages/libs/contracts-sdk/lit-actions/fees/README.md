# Fee Contract Lit Actions

This directory contains Lit Actions for fee-related operations in the Vincent Contracts SDK.

## Owner Attestation Signing

The `signOwnerAttestation` Lit Action provides an oracle service that:

1. Validates the provided `issuedAt` timestamp is within ±30 seconds of current time
...
4. Signs it using a PKP (Programmable Key Pair)

The `issuedAt` timestamp is generated by the caller and validated by each node to ensure all 10 parallel Lit nodes sign the exact same message, which is required for signature combining.
...
- `../generated/` - Build output (git-ignored, located at parent level)
...
In production, always use the Lit Action via the TypeScript SDK.
